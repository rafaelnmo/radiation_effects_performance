...
def version_insert(version):
	if version == 0:
		...
		with open(filepath, 'r+') as fp:
			...
			fp.seek(0)
			fp.truncate()
			linesVect[27] = 'Xinv_a buff_a buff_not_a vdd1 gnd inversor'
			linesVect[28] = 'Xinv_b buff_b buff_not_b vdd1 gnd inversor'
			linesVect[36] = 'Xxor_v1 buff_a buff_not_a buff_b buff_not_b xor vdd2 gnd xor_v1'
			linesVect[37] = '*Xxor_v2 buff_a buff_b xor vdd2 gnd xor_v2'
			linesVect[38] = '*Xxor_v3 buff_a buff_b xor vdd2 gnd xor_v3'
			linesVect[39] = '*Xxor_v4 buff_a buff_b xor vdd2 gnd xor_v4'
			linesVect[40] = '*Xxor_v5 buff_a buff_not_a buff_b buff_not_b xor vdd2 gnd xor_v5'
			linesVect[41] = '*Xxor_v6 buff_a buff_not_a buff_b buff_not_b xor vdd2 gnd xor_v6'
			linesVect[42] = '*Xxor_v7 buff_a buff_not_a buff_b buff_not_b xor vdd2 gnd xor_v7'
			linesVect[43] = '*Xxor_v8 buff_a buff_b xor vdd2 gnd xor_v8'
			linesVect[44] = '*Xxor_v9 buff_a buff_b buff_not_b xor vdd2 gnd xor_v9'
...
def increment_current(signal, currentValue):
	...
	if(signal == 0 or signal ==3):
		...
		with open(filepath, 'r+') as fp:
			...
			fp.seek(0)
			fp.truncate()
			linesVect[17] = 'Iset gnd xor EXP (0 '+str(currentValue)+'u 4ns 10ps 10ps 200ps)'
			# print(linesVect[14])
			linesVect[19] = '*Iset xor gnd EXP (0 29u 4ns 10ps 10ps 200ps)'
			x = '\n'.join(linesVect)
			#print(currentValue)
			fp.write(x)
			fp.close()
		...
		with open(filepath, 'r+') as fp:
			...
			fp.seek(0)
			fp.truncate()
			linesVect[17] = '*Iset gnd xor EXP (0 28u 4ns 10ps 10ps 200ps)'
			linesVect[19] = 'Iset xor gnd EXP (0 '+str(currentValue)+'u 4ns 10ps 10ps 200ps)'
			x = '\n'.join(linesVect)
			#print(currentValue)
			fp.write(x)
			fp.close()
